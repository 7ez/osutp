...
public class TpPerformance
{
    ...
    public TpPerformanceResult ComputeTotalValue()
    {
        ...
        var aim = ComputeAimValue();
        var speed = ComputeSpeedValue();
        var acc = ComputeAccValue();
        var attributes =
            Math.Pow(aim, 1.1f) +
            Math.Pow(speed, 1.1f) +
            Math.Pow(acc, 1.1f);

        var total = Math.Pow(attributes, 1.0f / 1.1f) * multiplier;
    ...
    {
        var aimValue = Math.Pow(5.0f * Math.Max(1.0f, Difficulty.AimStars / 0.0358f) - 4.0f, 3.0f) / 100000.0f;
        ...
        // Hidden Bonus
        if (Score.Mods.HasFlag(Mods.Hidden)) aimValue *= 1.18f;

        // Flashlight Bonus
        if (Score.Mods.HasFlag(Mods.Flashlight)) aimValue *= 1.36f;
    ...
    {
        var speedValue = Math.Pow(5.0f * Math.Max(1.0f, Difficulty.SpeedStars / 0.0358f) - 4.0f, 3.0f) / 100000.0f;
    ...
    public double ComputeAccValue()
    {
        ...
        if (Difficulty.AmountNormal > 0)
            betterAccuracyPercentage =
                (float)((Score.Amount300 - (Score.TotalHits() - Difficulty.AmountNormal)) * 6 + Score.Amount100 * 2 +
                        Score.Amount50)
        ...
        // Hidden Bonus
        if (Score.Mods.HasFlag(Mods.Hidden)) accValue *= 1.02f;

        // Flashlight Bonus
        if (Score.Mods.HasFlag(Mods.Flashlight)) accValue *= 1.02f;
